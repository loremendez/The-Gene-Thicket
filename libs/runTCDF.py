...
def runTCDF(gem, tf_target_pairs, cuda=False, epochs=500, kernel_size=4, levels=0, log_interval=100, lr=0.01, optimizername='Adam', seed=1, dilation_c=4, significance=0.8):
    ...
    links = pd.DataFrame()        
    columns = list(tf_target_pairs.keys())
    
    for i, key in enumerate(columns):
        if i % log_interval ==0 or i % len(columns) == 0 or i==0:
            percentage = (i/len(columns))*100
            print('Number of genes: {:2d} [{:.0f}%]'.format(i, percentage))
        target_name = key
        tfs_name = tf_target_pairs[key]
        if tfs_name != []:
            causes, realloss, scores_validated = TCDF.findcauses(target_name=target_name, tfs_name=tfs_name, gem=gem, cuda=cuda, epochs=epochs, kernel_size=kernel_size, layers=levels, log_interval=log_interval,lr=lr, optimizername=optimizername, seed=seed, dilation_c=dilation_c, significance=significance)
            #allscores[target_name]=scores
            #allscoresval[target_name]=scores_validated
            #allcauses[target_name]=causes
            #alldelays.update(causeswithdelay)
            #allreallosses[target_name]=realloss
            #allweights[i]=weights
            
            temp_df = pd.DataFrame({'TF':causes, 'importance':scores_validated})
            temp_df['target'] = str(key)
            links = links.append(temp_df)

    #return allcauses, allreallosses, allscores, allscoresval, columns
    return links
...
# def main(gem, tf_target_pairs): #, evaluation):
# #     if evaluation:
...
#         # run TCDF
#     allcauses, allreallosses, allscores, columns = runTCDF(gem, tf_target_pairs) #results of TCDF containing indices of causes and effects
...
#     pickle.dump(allcauses, open( "allcauses.p", "wb" ) )
#         #pickle.dump(alldelays, open( "alldelays.p", "wb" ) )
#     pickle.dump(allreallosses, open( "allrealosses.p", "wb" ) )
#     pickle.dump(allscores, open( "allscores.p", "wb" ) )
#     pickle.dump(columns, open( "columns.p", "wb" ) )
        #pickle.dump(allweights, open( "weights.p", "wb" ) )
...
# #group.add_argument('--tf_target_pairs', default={}, help="List of tf-targetgene pairs in the data set")
# #group.add_argument('--gem', default=False, help="gene expression matrix")
...
#     print("WARNING: The dilation coefficient is not equal to the kernel size. Multiple paths can lead to the same delays. Set kernel_size equal to dilation_c to have exactly one path for each delay.")
...
# tf_target_pairs=args.tf_target_pairs
